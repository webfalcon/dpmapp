...
class News extends Component {
    ...
    getApi(){
        return fetch(api+'posts?per_page=5&categories=10&page='+this.state.bookPage)
    ...
    render() {
        return (
            <Container>
                <PageHeader title="Նորություններ" {...this.props} />
                {this.state.ready ?
                    <Content>
                        <List dataArray={this.state.books} renderRow={(item)=>
                            <Body style={styles.listItem}>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate("SinglePage", [item, 'Նորություններ'])}>
