...
class Videos extends Component {
    ...
    getApi(){
        return fetch(api+'videos?per_page=5&page='+this.state.bookPage)
    ...
    render() {
        return (
            <Container>
                <PageHeader title="Տեսանյութեր" {...this.props} />
                {this.state.ready ?
                    <Content>
                        <List dataArray={this.state.books} renderRow={(item)=>
                            <Body style={styles.listItem}>
                            <TouchableOpacity onPress={() => this.props.navigation.navigate("SinglePage", [item, 'Տեսանյութեր'])}>
